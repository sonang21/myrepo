/* *************************************************************************
 * NAME : ASISTEST.UDP_DEAL_GOODS_INSERT_JCA
 * TYPE : PROCEDURE
 ...
 ************************************************************************* */
  ...
  CREATE OR REPLACE PROCEDURE "ASISTEST"."UDP_DEAL_GOODS_INSERT_JCA" (v_p IN VARCHAR,v_shopcode IN NUMBER,v_goodscode IN VARCHAR)
IS
BEGIN
    IF v_p = 'I' THEN
        IF v_shopcode = 806 THEN
          ...
          (
              SELECT DEAL_SEQ.NEXTVAL DEAL_IDX ,6688 DEAL_SHOP_CODE,GOODSCODE META_ID,SYSDATE START_AT,TO_DATE(TO_CHAR(SYSDATE,'yyyy-mm-dd'))+1-1/24/3600 END_AT,GOODSNM TITLE,REPLACE(URL,'?infl_cd=I0647','?infl_cd=I0031') URL,REPLACE(URL,'?infl_cd=I0647','?infl_cd=I0871') MOBILE_URL,PRICE PRICE,UDF_DEAL_CATE_MAPPING(CA_CODE) DIVISION,
