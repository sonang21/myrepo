/* *************************************************************************
 * NAME : DBENURI.UDP_FASHION_WEB_COUNT
 * TYPE : PROCEDURE
 * TIME : Create: 2018-03-28 18:50:00
 *        Modify: 2018-05-15 16:27:47
 *        Backup: 20180521_1739
 ************************************************************************* */


  CREATE OR REPLACE PROCEDURE "DBENURI"."UDP_FASHION_WEB_COUNT" 
IS
/*
...
CURSOR fashion_web_count_cursor IS
  ...
  WHERE P.SHOP_CODE = S.SHOP_CODE 
    AND P.MODELNO <= 0 
    AND (P.ca_code LIKE '14%' OR P.ca_code LIKE '15%' ) 
    AND P.status < '3' 
    AND P.srvflag = '0' 
    AND DECODE(p.esstockflag, NULL, '0', p.esstockflag) < '1'  
    AND (   P.soldflag IS NULL
         OR P.soldflag < 2  
         OR (P.soldflag = 2 AND (TO_CHAR(P.solddate, 'YYYYMMDDHH24') - TO_CHAR(SYSDATE, 'YYYYMMDDHH24')) <= 24) 
         OR (P.soldflag = 3 AND (TO_CHAR(P.solddate, 'YYYYMMDD') - TO_CHAR(SYSDATE, 'YYYYMMDD')) >= 8)
        )
    AND (P.IMGURL IS NOT NULL AND LENGTH(P.IMGURL) > 0 ) 
  GROUP BY rtrim(ca_code);
