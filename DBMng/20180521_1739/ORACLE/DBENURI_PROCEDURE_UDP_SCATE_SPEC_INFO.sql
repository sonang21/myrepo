/* *************************************************************************
 * NAME : DBENURI.UDP_SCATE_SPEC_INFO
 * TYPE : PROCEDURE
 * TIME : Create: 2018-03-28 18:50:01
 *        Modify: 2018-05-16 13:09:10
 *        Backup: 20180521_1739
 ************************************************************************* */


  CREATE OR REPLACE PROCEDURE "DBENURI"."UDP_SCATE_SPEC_INFO" AS
/*
...
BEGIN
  declare
   v_cate varchar2(12);
    ...
    begin
      open cur01;
        loop
          ...
          insert into TBL_SCATE_SPEC_INFO_TMP (SEARCH_CA_CODE,GPNO,GPSNO,SPEC_GROUP_SPECNO,SPEC_CATE_TITLE,COMMENTS,SPEC_CATE_AOFLAG,SPEC_CATE_DELFLAG,CATE,CATE_KB_NO,
          ...
          GROUP_NAME,STAIR_FLAG,HEIGHT,ISLEFTRIGHT,WIDTH,KB_NO,SPEC_CATE_SORTNO,SPEC_DETAIL_SORTNO,SPEC_GROUP_SORTNO,SPEC_CATE_ICON_TYPE,SPEC_CATE_ICON_EXPIRE_DATE,SPEC_GROUP_ICON_TYPE,SPEC_GROUP_ICON_EXPIRE_DATE) 
          SELECT v_cate,A.GPNO ,B.GPSNO,C.SPECNO SPEC_GROUP_SPECNO, A.TITLE SPEC_CATE_TITLE, A.COMMENTS, A.AOFLAG SPEC_CATE_AOFLAG, A.DELFLAG SPEC_CATE_DELFLAG,A.CATE,A.KB_NO CATE_KB_NO, 
                 ...
                 A.ICON_TYPE SPEC_CATE_ICON_TYPE ,A.EXPIRE_DATE SPEC_CATE_ICON_EXPIRE_DATE,C.ICON_TYPE SPEC_GROUP_ICON_TYPE,C.EXPIRE_DATE SPEC_GROUP_ICON_EXPIRE_DATE
            FROM TBL_SPEC_CATE A,
                 TBL_SPEC_CATEDETAIL B, 
                 TBL_SPEC_GROUP C
           WHERE A.CATE = substr(v_cate,1,4)
             AND A.GPNO = B.GPNO
             AND B.GPSNO = C.GPSNO
             AND B.GPNO = C.GPNO
             AND A.DELFLAG = '0'
    ...
    cursor cur02 is
    select search_ca_code,gpno,gpsno,spec_group_specno,spec_cate_title,spec_group_title,spec_group_delflag from tbl_scate_spec_info_tmp
          ...
          loop
            ...
            IF (v_spec_del_flag = '9' and v_spec_group_title <> 'blank') THEN
              IF (v_prv_spec_del_flag = '9' and v_prv_spec_group_title <> 'blank') then
                delete tbl_scate_spec_info_tmp where search_ca_code = v_prv_search_cate and gpno = v_prv_gpno and gpsno = v_prv_gpsno and spec_group_specno = v_prv_specno;
              END IF;
      ...
      commit;
      delete tbl_scate_spec_info;
      insert into tbl_scate_spec_info (select * from tbl_scate_spec_info_tmp);
      delete tbl_scate_spec_info_tmp;
      if (sqlcode < 0) then
        rollback;
      else
        commit;
