 ...
 ************************************************************************* */
  ...
  CREATE OR REPLACE PROCEDURE "DEAL"."COUPON_CR_ONE" 
(
...
declare
   ...
   cursor cur01 is
       select nvl(a.cp_pid,' ')
             ...
             /*
             ,case when b.deal_shop_code = 4027 then 'http://banner.auction.co.kr/bn_redirect.asp?ID=BN00174818' || '&' || 'lpurl=http://itempage3.auction.co.kr/DetailView.aspx?itemno=' || trim(b.meta_id)
                   when b.deal_shop_code = 6508 then 'http://www.wemakeprice.com/widget/aff_bridge_public/socialmoa/' || trim(b.meta_id) || '/Y/META_af'
             ...
             ,case when b.deal_shop_code = 6688 then nvl(b.url,' ')
                   when b.deal_shop_code = 6508 then 'http://www.wemakeprice.com/widget/aff_bridge_public/socialmoa/' || trim(b.meta_id) || '/Y/META_af'
                   when b.deal_shop_code = 4027 then 'http://banner.auction.co.kr/bn_redirect.asp?ID=BN00174819' || '&' || 'lpurl=http://itempage3.auction.co.kr/DetailView.aspx?itemno=' || trim(b.meta_id)
                   when b.deal_shop_code = 6641 then 'http://www.ticketmonster.co.kr/entry/?jp=80025' || '&' || 'ln=212749' || '&' || 'p_no=' || trim(b.meta_id)
...
dbms_output.put_line('insert end deal_shop_code = ' || T_MB_ID || ' ' || 'meta_id=' || T_CP_PID );
--d4_coupon image5 update
update d4_coupon set cp_image5 = cp_image1 where (cp_image5 = ' ' or cp_image5 is null) and cp_company = v_shop_code and cp_pid = T_CP_PID; 

update d4_coupon set cp_salerate = 0 where cp_salerate < 0 and cp_company = v_shop_code and cp_pid = T_CP_PID; 
...
where cp_company = v_shop_code and cp_pid = T_CP_PID;
...
where cp_cate1 <> ' ' and a.cp_company = v_shop_code and cp_pid = T_CP_PID;                                        
...
update d4_coupon set cp_cate1 = ' ' where cp_cate2 = ' ' and cp_company = v_shop_code and cp_pid = T_CP_PID;
...
declare
   ...
   cursor cur03 is
      ...
      and (a.cp_address is not null and a.cp_address <> ' ' and a.cp_address <> '전국배송')
