/* *************************************************************************
 * NAME : DBENURI.UDP_GOODS_ALLNEW5
 * TYPE : PROCEDURE
 * TIME : Create: 2018-03-28 18:50:00
 *        Modify: 2018-05-15 16:37:11
 *        Backup: 20180521_1739
 ************************************************************************* */


  CREATE OR REPLACE PROCEDURE "DBENURI"."UDP_GOODS_ALLNEW5" (
...
IS
     CURSOR cur_goods_info IS
       SELECT a.modelno, NVL(a.modelno_group,0) modelno_group, 
              b.minprice, NVL(b.minprice2,0) minprice2 ,NVL(b.minprice3,0) minprice3 , NVL(b.minprice4,0) minprice4 ,b.avgprice, b.maxprice, b.mallcnt, NVL(b.mallcnt3,0) mallcnt3, b.popular, b.sale_cnt,
              substr(a.ca_code,0,2) ca_lcode, a.ca_code,
              NVL(a.p_pl_no,0) p_pl_no,NVL(a.p_imgurl,' ') p_imgurl,NVL(a.p_imgurl2,' ') p_imgurl2,NVL(a.p_imgurlflag,' ') p_imgurlflag, NVL(a.imgchk,'0') imgchk, a.constrain, 
              NVL(b.bbs_num,0) bbs_num, NVL(b.bbs_point,0) bbs_point, NVL(b.bbs_point_avg,0) bbs_point_avg
         FROM TBL_GOODS a, TBL_GOODS_SUM b
        WHERE a.modelno = b.modelno
          AND a.ca_code like v_ca_lcode||'%'
          AND (b.mallcnt > 0 OR (b.mallcnt=0 AND a.moddate > (sysdate-5)))
          AND a.modelno > 0
          AND a.JOBCODE > '0'
         ;
     ...
     v_minprice_tmp      tbl_goods_sum.minprice%TYPE := 0;
     v_minprice2_tmp     tbl_goods_sum.minprice2%TYPE := 0;
     v_minprice3_tmp     tbl_goods_sum.minprice3%TYPE := 0;
     v_minprice4_tmp     tbl_goods_sum.minprice4%TYPE := 0;
     v_avgprice_tmp      tbl_goods_sum.avgprice%TYPE := 0;
     v_maxprice_tmp      tbl_goods_sum.maxprice%TYPE := 0;
     v_mallcnt_tmp       tbl_goods_sum.mallcnt%TYPE := 0;
     v_mallcnt3_tmp      tbl_goods_sum.mallcnt3%TYPE := 0;
     v_popular_tmp       tbl_goods_sum.popular%TYPE := 0;
     v_sale_cnt_tmp      tbl_goods_sum.sale_cnt%TYPE := 0;
     v_bbs_num_tmp       tbl_goods_sum.bbs_num%TYPE := 0;
     v_bbs_point_tmp     tbl_goods_sum.bbs_point%TYPE := 0;
     v_bbs_point_avg_tmp tbl_goods_sum.bbs_point_avg%TYPE := 0;

     v_ca_code_tmp       tbl_cate_goods.ca_code%TYPE := '';
     v_ca_lcode_tmp      tbl_cate_goods.ca_lcode%TYPE := '';
     ...
     v_minprice2         tbl_goods_sum.minprice2%TYPE := 0;
     v_minprice3         tbl_goods_sum.minprice3%TYPE := 0;
     v_minprice4         tbl_goods_sum.minprice4%TYPE := 0;
     v_avgprice          tbl_goods_sum.avgprice%TYPE := 0;
     v_maxprice          tbl_goods_sum.maxprice%TYPE := 0;
     v_mallcnt           tbl_goods_sum.mallcnt%TYPE := 0;
     v_mallcnt3          tbl_goods_sum.mallcnt3%TYPE := 0;
     v_popular           tbl_goods_sum.popular%TYPE := 0;
     v_sale_cnt          tbl_goods_sum.sale_cnt%TYPE := 0;
     v_bbs_num           tbl_goods_sum.bbs_num%TYPE := 0;
     v_bbs_point         tbl_goods_sum.bbs_point%TYPE := 0;
     v_bbs_point_avg     tbl_goods_sum.bbs_point_avg%TYPE := 0;
        ...
        END;
          ...
          IF v_minprice <> v_minprice_tmp OR nvl(v_minprice2,0) <> v_minprice2_tmp OR v_minprice3 <> v_minprice3_tmp
              ...
              IF v_constrain_tmp='5' THEN --유사상품
                  ...
                  UPDATE /*+ append NOLOGGING */ TBL_GOODS_SUM
                  ...
                  UPDATE /*+ append NOLOGGING */ TBL_GOODS
                     set p_pl_no      = case when (v_p_imgurl <> ' ' AND v_p_imgurlflag <> ' ') then v_p_pl_no else p_pl_no end,
