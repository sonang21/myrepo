/* *************************************************************************
 * NAME : DBENURI.UDP_CATE_POPULAR_KEYWORD
 * TYPE : PROCEDURE
 * TIME : Create: 2018-03-28 18:50:00
 *        Modify: 2018-05-15 13:23:55
 *        Backup: 20180521_1739
 ************************************************************************* */


  CREATE OR REPLACE PROCEDURE "DBENURI"."UDP_CATE_POPULAR_KEYWORD" 
IS
/*
        ...
        FOR tmp_loop2 in (
          select A.keyword,A.cate from ( 
            ...
            ( 
              ...
              ( 
                select V.keyword,V.modelno,substr(G.ca_code,0,2) ca_code
                  from TBL_SEARCH_KEYWORD_VIEW_LOG V,TBL_GOODS G  
                 where V.reg_date>=to_date(sysdate-30, 'yyyy-mm-dd') 
                   and V.keyword = v_keyword
                   and V.view_type = '1'
                   and V.modelno = G.modelno
                   and V.modelno > 0 
              ) 
              ...
              select keyword,ca_code as cate,count(*) as cnt from 
              ( 
                select V.keyword,V.modelno,substr(trim(P.ca_code),0,2) ca_code
                  from TBL_SEARCH_KEYWORD_VIEW_LOG V,TBL_PRICELIST P  
                 where V.reg_date>=to_date(sysdate-30, 'yyyy-mm-dd')
                   and V.keyword = v_keyword
                   and V.view_type = '2'
                   and V.pl_no = P.pl_no
