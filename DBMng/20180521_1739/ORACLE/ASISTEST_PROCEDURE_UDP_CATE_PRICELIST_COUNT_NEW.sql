/* *************************************************************************
 * NAME : ASISTEST.UDP_CATE_PRICELIST_COUNT_NEW
 * TYPE : PROCEDURE
 ...
 ************************************************************************* */
  ...
  CREATE OR REPLACE PROCEDURE "ASISTEST"."UDP_CATE_PRICELIST_COUNT_NEW" AS
BEGIN
  ...
  from (
        ...
        SELECT  /*+ full(P2) parallel(P2 8) full(G) parallel(G 4)*/
          ...
          FROM   dbenuri.TBL_PRICELIST P2    
                ...
                ON P2.MODELNO = G.MODELNO AND G.CATEFLAG='0' AND P2.MODELNO>0 
         WHERE P2.SRVFLAG in ('0','L','R','Z','2','3','6','7','9','O','D','S','P')  
              ...
              AND (  ( G.CONSTRAIN='1' AND G.JOBCODE = '0' )        
                ...
                OR (P2.option_flag2='1' AND G.minprice < P2.price))        
...
HAVING grouping(cate4) = 0  -- Áß
        OR (grouping(cate6) = 0 AND length(cate6)=6 ) -- ¼Ò
        OR (grouping(cate8) = 0 AND length(cate8)=8 ); -- ¹Ì
commit;
declare
    v_cate varchar2(12);
    v_cnt  number(10);
    cursor cur01 is
      select ca_code,count from TBL_CATE_PRICELIST_COUNT_TMP ;
    begin
      open cur01;
        loop
          fetch cur01      
          ...
          delete TBL_CATE_PRICELIST_COUNT where CA_CODE =  v_cate ;
          insert into TBL_CATE_PRICELIST_COUNT (CA_CODE,COUNT) values (v_cate,v_cnt);  
          commit;
        end loop;
      close cur01;
    end;
UDP_CATE_PRICELIST_M_COUNT_NEW();
END UDP_CATE_PRICELIST_COUNT_NEW;