/* *************************************************************************
 * NAME : DBADM.UP_MIG_TBL_GOODS
 * TYPE : PROCEDURE
 * TIME : Create: 2018-04-02 12:53:58
 *        Modify: 2018-05-10 13:22:31
 *        Backup: 20180521_1739
 ************************************************************************* */


  CREATE OR REPLACE PROCEDURE "DBADM"."UP_MIG_TBL_GOODS" (P_VALUE VARCHAR2 DEFAULT NULL)
-- =============================================================================
-- 대상테이블 : TBL_GOODS (MIG_GOODS)
-- 소스테이블 : TBL_GOODS (OLD)
-- 설  명 : 
--------------------------------------------------------------------------------
-- 작성자 : 송정훈
    ...
    ******************************************************************************* */  
    ...
    INSERT /*+ APPEND */
      INTO dbenuri.MIG_GOODS
           ...
           ------------------------ 45
           , CA_CODE                -- 5.9 컬럼추가
           , SUM_MODDATE            -- 5.9 컬럼추가 
           , CATE_MODDATE           -- 5.9 컬럼추가 
           , CHANGE_MODEDATE        -- 5.9 컬럼추가 
           )
    SELECT /*+ PARALLEL(4) */
           A.MODELNO
         , MAX(A.RECOMMEND        )
         ...
         , MAX(A.B.UNIT           )
           ------------------------ 45
         , MAX(DECODE(A.CATEFLAG,'0',CA_CODE))  CA_CODE                -- 5.9 컬럼추가
         , MAX(A.MODDATE          )             SUM_MODDATE            -- 5.9 컬럼추가 
         , MAX(A.MODDATE          )             CATE_MODDATE           -- 5.9 컬럼추가 
         , MAX(A.MODDATE          )             CHANGE_MODEDATE        -- 5.9 컬럼추가 
