/* *************************************************************************
 * NAME : ES_APP.MIG_TMP_PRICE_DATA_P00
 * TYPE : PROCEDURE
 * TIME : Create: 2018-04-13 14:49:25
 *        Modify: 2018-04-25 11:25:22
 *        Backup: 20180521_1739
 ************************************************************************* */


  CREATE OR REPLACE PROCEDURE "ES_APP"."MIG_TMP_PRICE_DATA_P00" 
( V_MOD_NO   NUMBER ,  -- 6등분 번호(0~5)
  V_PARL_NO  NUMBER    -- PARALLEL 갯수 
)
IS
...
BEGIN
       ...
       EXE_SQL := 'INSERT /*+ APPEND NOLOGGING */ INTO ES_APP.TMP_PRICE_DATA_01 PARTITION FOR ('||V_MOD_NO||')
                  ( ID                     
       ...
       'SELECT /*+ PARALLEL('||V_PARL_NO||') */
            ...
            popular2,
            sale_cnt,
            CASE WHEN SERVICE_GUBUN = 1 THEN UDF_MODEL_FACTORY_ALL(REPLACE(MODELNM || '' '' || FACTORY, ''-'', '''')) ELSE '''' END,
            minprice,
            maxprice,
            minprices,
            C_DATE,
        ...
        FROM (
               SELECT 
                    ...
                    , CA_DEPT_CODE
                    , case
                        when length(CA_CODE)>= 2 then
                          substr(CA_CODE, 1,2) 
                      end as CA_LCODE_RAN
                    , case
                        when length(CA_CODE)>= 4 then
                          substr(CA_CODE, 1,4) 
                      end as CA_MCODE_RAN
                    , case
                        when length(CA_CODE)>= 6 then
                          substr(CA_CODE, 1,6) 
                      end as CA_SCODE_RAN
                    , case
                        when length(CA_CODE)>= 8 then
                          substr(CA_CODE, 1,8) 
                      end as CA_DCODE_RAN
                    , MODELNM
                    , MODELNM2
         ...
         ) A LEFT JOIN ES_APP.TMP_ES_PRICE_CATE_01 B ON (B.CATEGORY = A.CATEGORY) 
     ';
