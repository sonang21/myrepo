 ...
 ************************************************************************* */
  ...
  CREATE OR REPLACE PROCEDURE "ASISTEST"."UDP_CATE_GOODS" 
is
/******************************************************************************
**  Name: UDP_CATE_GOODS
...
begin
  ...
  l_sql := 'CREATE TABLE TMPT_MCSS_GOODS  partition by hash(mod_20) partitions 20  AS
            SELECT MODELNO, CA_CODE, MODELNO_GROUP, MODELNM, CONDINAME, FACTORY, BRAND, CONSTRAIN, IMGCHK3, MALLCNT, MINPRICE, MAXPRICE, AVGPRICE, SPEC, KEYWORD2, FUNC, POPULAR, C_DATE, MODDATE, mod(rownum, 20) as mod_20
              FROM TBL_GOODS,
             ...
             WHERE CA_LCODE = SUBSTR(CA.CATEGORY,0,2)
               AND CA_CODE = CA.CATEGORY
               AND CATEFLAG = ''0'' AND MALLCNT > 0 AND JOBCODE > ''0'' AND CONSTRAIN IN (''1'',''5'')';
  -- dbms_output.put_line(l_sql);
  execute immediate (l_sql);
