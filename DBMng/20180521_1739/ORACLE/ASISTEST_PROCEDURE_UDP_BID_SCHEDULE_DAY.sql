/* *************************************************************************
 * NAME : ASISTEST.UDP_BID_SCHEDULE_DAY
 * TYPE : PROCEDURE
 ...
 ************************************************************************* */
  ...
  CREATE OR REPLACE PROCEDURE "ASISTEST"."UDP_BID_SCHEDULE_DAY" (SDATE IN VARCHAR2)
IS
...
BEGIN
    ...
    --sys log
    INSERT INTO TBL_SYS_PROC_LOG (NAME, STATUS) VALUES ('UDP_BID_SCHEDULE_DAY','START');
    COMMIT;
    ...
    DELETE TBL_BID_DAILY WHERE BID_DATE = '20141216';
    COMMIT;
    ...
    FOR tmp_loop in (
                     SELECT PL_NO as pl_no, BID_UNIT_PRICE as bid_unit_price
         				     FROM TBL_BID_GOODS_EXP
 			          		 WHERE BID_STATUS = '1'
      				       OR (BID_STATUS = '3' AND to_char(BID_END_DATE,'YYYYMMDD') = '20141216')
      				       UNION
      				       SELECT PL_NO as pl_no, UNIT_PRICE  as bid_unit_price
      				       FROM TBL_BID_REPORT
      				       WHERE to_char(REG_DATE,'YYYYMMDD') = '20141216'
      				       UNION
          					 ...
          					 AND to_char(UNIT_LOG_DATE,'YYYYMMDD') = '20141216'
                    ) LOOP
	   BEGIN
			      ...
			      FOR tmp_loop2 in (SELECT DEDUCTION_IDX
			                        ...
			                        AND DEDUCTION_SEARCH_DATE = '20141216'
	          ...
	          -- 지난 총 소진금액 구하기
		        ...
		        AND bid_unit_price = tmp_loop.bid_unit_price;
		          ...
		          SELECT COUNT(CASE WHEN (BID_TYPE = 'U' OR BID_TYPE = 'I') THEN LOG_IDX END) INTO v_bid_cnt
					    FROM TBL_BID_LOG
	          	WHERE to_char(BID_REG_DATE,'YYYYMMDD') = '20141216'
	          	AND BID_UNIT_PRICE = tmp_loop.bid_unit_price
	          	AND PL_NO = tmp_loop.pl_no;
		        ...
		        FROM TBL_BID_UNIT_LOG
		        WHERE to_char(UNIT_LOG_DATE,'YYYYMMDD') = '20141216'
			    ...
			    VALUES(v_no, '20141216', tmp_loop.pl_no, v_ca_code, v_modelno, v_bid_valid_view, v_bid_total_view, v_bid_possess_money, v_bid_all_price,v_bid_exhaustion_money, v_bid_unit_price, v_bid_cnt, v_service_cnt, v_bid_total_click, v_bid_valid_click);
    ...
    --sys log
    INSERT INTO TBL_SYS_PROC_LOG (NAME, STATUS) VALUES ('UDP_BID_SCHEDULE_DAY','END');
    COMMIT;
    
END UDP_BID_SCHEDULE_DAY;
