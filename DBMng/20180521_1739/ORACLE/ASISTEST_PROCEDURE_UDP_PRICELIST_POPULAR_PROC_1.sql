/* *************************************************************************
 * NAME : ASISTEST.UDP_PRICELIST_POPULAR_PROC_1
 * TYPE : PROCEDURE
 ...
 ************************************************************************* */
  ...
  CREATE OR REPLACE PROCEDURE "ASISTEST"."UDP_PRICELIST_POPULAR_PROC_1" (
...
BEGIN
    ...
    DELETE TBL_PRICELIST_POPULAR WHERE CA_CODE like '0914%' OR CA_CODE like '0311%' OR CA_CODE like '16%';
    COMMIT;

    INSERT /*+ PARALLEL(TBL_PRICELIST_POPULAR, 6) */ INTO TBL_PRICELIST_POPULAR (PL_NO, POPULAR,CA_CODE)
    SELECT /*+ full(P) parallel(P 6) */
    			 P.PL_NO,
       ...
       AND (p.ca_code like '0914%' OR p.ca_code like '0311%' OR p.ca_code like '16%');
       --AND (p.ca_code NOT like '13%' AND p.ca_code NOT like '9804%' AND p.ca_code NOT like '14%' AND p.ca_code NOT like '15%');
    ...
    --===============================================================================================================
    i_count := 0;
    FOR tmp_loop in (SELECT PL_NO, POPULAR FROM TBL_PRICELIST_POPULAR WHERE CA_CODE like '0914%' OR CA_CODE like '0311%' OR CA_CODE like '16%') LOOP
    ...
    --================================================================================================================
  	...
  	endTime   := '' || '|1 : ' || tmp_end;
   	...
   	IF result_desc = '' THEN
    	result_desc := 'UDP_PRICELIST_POPULAR_PROC_1 SUCCESS ';
   	ELSE
    	result_desc := result_desc || '|' || 'UDP_PRICELIST_POPULAR_PROC_1 SUCCESS ';
   	END IF;
...
EXCEPTION
    ...
    IF result_desc = '' THEN
         result_desc := 'UDP_PRICELIST_POPULAR_PROC_1 => ERROR : ' || SQLERRM;
    ELSE
         result_desc := result_desc || '|' || 'UDP_PRICELIST_POPULAR_PROC_1 => ERROR : ' || SQLERRM;
    END IF;

		endTime   := endTime || '|1 error: ' || tmp_end;

END UDP_PRICELIST_POPULAR_PROC_1;