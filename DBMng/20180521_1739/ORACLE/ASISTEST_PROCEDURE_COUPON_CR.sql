 ...
 ************************************************************************* */
  ...
  CREATE OR REPLACE PROCEDURE "ASISTEST"."COUPON_CR" 
...
declare
   ...
   cursor cur01 is
       select nvl(a.cp_pid,' ')
             ...
             ,case when (1 - nvl(round(to_number(to_char(((to_number(case when (b.price = ' ' OR b.price = '0') then '0' when (b.original = ' ' OR b.original = '0') then '0' else b.price end,'99999999999')) /
             (to_number(case when (b.original = ' ' or b.original = '0' or b.original = '0.0' or b.original is null) then '100' else b.original end,'99999999999'))),'99999999999.99999')),2),0)) * 100 = 100 then 0
              else   (1 - nvl(round(to_number(to_char(((to_number(case when (b.price = ' ' OR b.price = '0') then '0' when (b.original = ' ' OR b.original = '0') then '0' else b.price end,'99999999999')) /
             ...
             ,nvl(to_number(b.max_count,'99999999999'),0)
             ...
             ,nvl((select c.dbenuri.deal_sale_shop_address from dbenuri.deal_sale_shop c
                      where b.deal_idx = c.deal_idx
                        and c.dbenuri.deal_sale_shop_idx = (select max(dbenuri.deal_sale_shop_idx)
...
select to_number(cp_id.nextval,'9999999999')
      ...
      (to_number(case when (b.original = ' ' or b.original = '0' or b.original = '0.0' or b.original is null) then '100' else b.original end,'99999999999'))),'99999999999.99999')),2),0)) * 100 end            
      ,nvl(to_number(case when (b.max_count = ' ') then '0' else b.max_count end,'99999999999'),0)
      ...
      ,0
      ,'Y'
      ,nvl((select to_number(trim(aa.dbenuri.deal_sale_shop_longigude))
          from dbenuri.deal_sale_shop aa
          where aa.deal_idx = b.deal_idx
          and ISNUMERIC(dbenuri.deal_sale_shop_longigude) = 1
          and aa.dbenuri.deal_sale_shop_idx = (select max(dbenuri.deal_sale_shop_idx)
                                         from dbenuri.deal_sale_shop aaa
                                         where aa.deal_idx = aaa.deal_idx)),0)
      ,nvl((select to_number(trim(aa.dbenuri.deal_sale_shop_latitude))
          from dbenuri.deal_sale_shop aa
          where aa.deal_idx = b.deal_idx
          and ISNUMERIC(dbenuri.deal_sale_shop_latitude) = 1
          and aa.dbenuri.deal_sale_shop_idx = (select max(dbenuri.deal_sale_shop_idx)
                                         from dbenuri.deal_sale_shop aaa
                                         where aa.deal_idx = aaa.deal_idx)),0)
      ,0
      ...
      ,0
      ,0
      ,nvl((select aa.dbenuri.deal_sale_shop_name
          from dbenuri.deal_sale_shop aa
          where aa.deal_idx = b.deal_idx
          and aa.dbenuri.deal_sale_shop_idx = (select max(dbenuri.deal_sale_shop_idx)
                                         from dbenuri.deal_sale_shop aaa
                                         where aa.deal_idx = aaa.deal_idx)),' ')
      ,nvl((select aa.dbenuri.deal_sale_shop_tel
          from dbenuri.deal_sale_shop aa
          where aa.deal_idx = b.deal_idx
          and aa.dbenuri.deal_sale_shop_idx = (select max(dbenuri.deal_sale_shop_idx)
                                         from dbenuri.deal_sale_shop aaa
...
declare
   ...
   cursor cur03 is
      ...
      where substr(a.cp_lastupdate,1,10) = to_char(sysdate,'yyyy-mm-dd')
      and a.cp_pid = b.meta_id
      and b.deal_idx = c.deal_idx
      and b.deal_shop_code = T_MB_ID
      ...
      loop
        ...
        begin
          ...
          loop
            ...
            if ((v_loccate <> t_loccate) and (t_loccate <> ' ')) then
              if (v_true = '1') then
                 ...
                 if (v_rowcnt = 0) then
                   ...
                   if (sqlcode < 0) then
                      o_err_msg := 'd4_coupon_loc insert error (' || v_cp_id || ': ' || t_loccate || ': ' || sqlcode || ')';
                      chk_err_fg := '1';
                   end if;
            ...
            if ((v_cp_address like '%' || v_keyword || '%') and v_opt = 'or') then
              v_true := '1';
            end if;
            
            if ((v_cp_address not like '%' || v_keyword || '%') and v_opt = 'and') then
              v_true := '0';
            end if;
...
declare
   ...
   cursor cur05 is
     SELECT c.cp_id
            ,a.dbenuri.deal_sale_shop_name
            ,a.dbenuri.deal_sale_shop_tel
            ,case when ISNUMERIC(a.dbenuri.deal_sale_shop_latitude) = 1 then a.dbenuri.deal_sale_shop_latitude else '0' end
            ,case when ISNUMERIC(a.dbenuri.deal_sale_shop_longigude) = 1 then a.dbenuri.deal_sale_shop_longigude else '0' end
            ,a.dbenuri.deal_sale_shop_address
      ...
      and b.deal_shop_code = T_MB_ID
      order by c.cp_id,a.dbenuri.deal_sale_shop_name;
