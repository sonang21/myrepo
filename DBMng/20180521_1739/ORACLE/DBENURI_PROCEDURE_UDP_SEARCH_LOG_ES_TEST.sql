...
BEGIN
    ...
    -------------------------------------------------------------------------------------------------------------------------------------------------
    -- 0508 추가색인용 상품 자료 생성 (CK_03.SQL, CK_04.SQL 참고)
    -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    --   - 테이블에 없는 값 필요시 TBL_ES_SEARCH_LOG 컬럼 추가, 트리거 수정하여 추가된 컬럼에 값 반영 검토 필요 
    -- 2.테스트 완료후 TBL_SEARCH_LOG_TMP -> TBL_SEARCH_LOG 변경, NOLOGGING 제거 해야 함 
    -- 3.TBL_ES_SEARCH_LOG 완료 처리된 데이터 백업/삭제 방법 추가 
    -- 4.사용되는 function 중 ES_APP로 된 것 반영 검토 (★★★★★)
    -------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            BEGIN
                 ...
                 SELECT  /*+ PARALLEL(2) */   -- /*+ PARALLEL('||V_PARL_NO||') */
                      ...
                      , CASE WHEN SUBSTR(P.CA_CODE,0,4) = '9501' THEN 
                                  UDF_GOODSNM_REMOVE(P.goodsnm) || ' 음반 앨범' 
                      ...
                      , ' '                                                                                                          AS SPECOPT
                      , '@'                                                                                                          AS SERVICE_FLAG
                      , E.STATUS                                                                                                     AS STATUS          -- 확인 
                      , E.REGDATE                                                                                                    AS REGDATE         -- 확인 
                      , CASE WHEN P.CA_CODE IS NULL          THEN '000000' 
                             WHEN LENGTH(RTRIM(P.CA_CODE))=2 THEN RTRIM(P.CA_CODE) || '0000' 
                             WHEN LENGTH(RTRIM(P.CA_CODE))=4 THEN RTRIM(P.CA_CODE) || '00' 
                             ELSE P.CA_CODE 
                      ...
                      , CASE WHEN SUBSTR(P.CA_CODE,0,4) = '1219' THEN 
                             ...
                             END                                                                                                     AS MODELNM2
                      , 0                                                                                                            AS WEIGHT
                      , '@'                                                                                                          AS SC_WEB          -- 확인  UDF_SC_CHECK('0', v_modelno, '1', v_shop_code)
                      , '@'                                                                                                          AS SC_MOBILE       -- 확인  UDF_SC_CHECK('0', v_modelno, '2', v_shop_code)
                 ...
                 SELECT  /*+ PARALLEL(2) */
                      ...
                      , '@'                                                                                                         AS SERVICE_FLAG
                      , MAX(E.STATUS )                                                                                              AS STATUS          -- 확인 MAX
                      , MAX(E.REGDATE)                                                                                              AS REGDATE         -- 확인 MAX
                  ...
                  FROM  TBL_GOODS                         G  -- PARTITION 단위 처리 고려 
                        INNER JOIN      TBL_ES_SEARCH_LOG E ON G.MODELNO   = E.MODELNO  AND E.SL_APPLY_FLAG = '2' AND E.SL_APPLY_SID  = P_SID 
                                                                                                                -- 0508 확인 필요 -> MODELNO 있는 놈 만 ? 
                        LEFT OUTER JOIN TBL_GOODS_SUM     M ON G.MODELNO   = M.MODELNO                          -- 0508 확인 필요 
                        LEFT OUTER JOIN TBL_CATE_GOODS    C ON G.MODELNO   = C.MODELNO  AND C.CATEFLAG = '0'    -- 0508 확인 필요 
                  WHERE jobcode >= '1'
                    AND (constrain = '1' OR (constrain = '5' AND mallcnt > 0))
                    AND  cateflag ='0'
                  GROUP BY nvl(modelno_group,G.modelno)
                --ORDER BY E.REGDATE                                                                            -- 0510 확인 : ORDER BY or DISTINCT MODELNO, TBL_PRICELIST도 
                  ;
