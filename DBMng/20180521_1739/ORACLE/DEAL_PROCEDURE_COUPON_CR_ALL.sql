 ...
 ************************************************************************* */
  ...
  CREATE OR REPLACE PROCEDURE "DEAL"."COUPON_CR_ALL" 

(
T_MB_ID IN D4_COUPON.MB_ID%TYPE
)
----------------------------------------------------------------------------------------------------------------------
-- File Name : COUPON_CR_ALL
-- Version      : 1.0
-- Writen Date  : 2014-11-19
-- Author    : Kwon Sung Won
-- Program ID  : COUPON_CR_ALL
...
update d4_coupon set cp_image5 = cp_image1 where cp_image5 = ' ' or cp_image5 is null; 
commit;
update d4_coupon set cp_salerate = 0 where cp_salerate < 0;
commit;
...
where cp_cate1 <> ' ';  
commit;
update d4_coupon set cp_cate1 = ' ' where cp_cate2 = ' ';
commit;
dbms_output.put_line(o_err_msg || ' : ' || chk_err_fg);
          ...
          loop
            ...
            if (v_loccate <> t_loccate) and (t_loccate <> ' ') then
              if (v_true = '1' or v_true = '2') then
                 ...
                 */
                 delete d4_coupon_loc where cp_id = v_cp_id;
                 insert into d4_coupon_loc values (v_cp_id,t_loccate,(select b.pcode from d4_commoncode b where b.code = t_loccate));
              end if;
              v_true := '0';
            end if;
...
if (chk_err_fg = '1') then
  ...
  dbms_output.put_line('COUPON_CR_ALL SP EXCUTED SUCCESSSFULLY!!'); 
  commit;     
end if;

EXCEPTION
WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE( SQLERRM || o_err_msg || ' ' || 'register is failed!');
