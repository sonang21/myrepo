/* *************************************************************************
 * NAME : DBENURI.TR_SEN_UPPRICELIST
 * TYPE : TRIGGER
 * TIME : Create: 2018-04-18 18:39:10
 *        Modify: 2018-05-21 17:29:10
 *        Backup: 20180521_1739
 ************************************************************************* */


  CREATE OR REPLACE TRIGGER "DBENURI"."TR_SEN_UPPRICELIST" AFTER UPDATE ON dbenuri.TBL_PRICELIST
...
DECLARE
       ...
       v_avgprice_new            tbl_goods_sum.avgprice%TYPE     := 0; --0418 tbl_goods --> tbl_goods_sum
       ...
       v_ap_minprice_new         tbl_goods_sum.minprice%TYPE     := 0; --0418 tbl_goods --> tbl_goods_sum 

       v_socialprice_new         tbl_goods_sum.socialprice%TYPE  := 0; --0418 tbl_goods --> tbl_goods_sum
       v_maxprice_new            tbl_goods_sum.maxprice%TYPE     := 0; --0418 tbl_goods --> tbl_goods_sum
       v_mallcnt_new             tbl_goods_sum.mallcnt%TYPE      := 0; --0418 tbl_goods --> tbl_goods_sum
       v_mallcnt3_new            tbl_goods_sum.mallcnt3%TYPE     := 0; --0418 tbl_goods --> tbl_goods_sum
       v_avgprice_old            tbl_goods_sum.avgprice%TYPE     := 0; --0418 tbl_goods --> tbl_goods_sum
       v_minprice_old            tbl_goods_sum.minprice%TYPE     := 0; --0418 tbl_goods --> tbl_goods_sum
       -------------------------------------------------------------------------------------------------------------------
       -- 배송비 포함 최저가 nova23 2016.06.21
       -------------------------------------------------------------------------------------------------------------------
       v_minprice2_old           tbl_goods_sum.minprice2%TYPE    := 0; --0418 tbl_goods --> tbl_goods_sum
       v_minprice3_old           tbl_goods_sum.minprice3%TYPE    := 0; --0418 tbl_goods --> tbl_goods_sum
       v_minprice4_old           tbl_goods_sum.minprice4%TYPE    := 0; --0418 tbl_goods --> tbl_goods_sum

       v_maxprice_old            tbl_goods_sum.maxprice%TYPE     := 0; --0418 tbl_goods --> tbl_goods_sum
       v_mallcnt_old             tbl_goods_sum.mallcnt%TYPE      := 0; --0418 tbl_goods --> tbl_goods_sum
       v_mallcnt3_old            tbl_goods_sum.mallcnt3%TYPE     := 0; --0418 tbl_goods --> tbl_goods_sum
...
--0418 v_searchflag_old          tbl_pricelist.searchflag%TYPE   := '';
--0418 v_searchflag_new          tbl_pricelist.searchflag%TYPE   := '';
       v_goodsfactory_old        tbl_pricelist.goodsfactory%TYPE := '';
       v_goodsfactory_new        tbl_pricelist.goodsfactory%TYPE := '';
--0418 v_goodsbrand_old          tbl_pricelist.goodsbrand%TYPE   := '';
--0418 v_goodsbrand_new          tbl_pricelist.goodsbrand%TYPE   := '';  
...
--0418 v_goodsbrand              VARCHAR2(100)  := v_goodsbrand_new;
...
BEGIN
    ...
    LOOP
        IF i <= pg_Pricelist_DataLoop.p_index THEN

       v_modelno_old           :=  NVL(pg_Pricelist_DataLoop.v_Modelno_old(i),0);
       v_modelno_new           :=  NVL(pg_Pricelist_DataLoop.v_Modelno_new(i),0);
       v_pl_no_old             :=  pg_Pricelist_DataLoop.v_Pl_no_old(i);
       v_pl_no_new             :=  pg_Pricelist_DataLoop.v_Pl_no_new(i);
       v_shop_code_old         :=  pg_Pricelist_DataLoop.v_Shop_code_old(i);
       v_shop_code_new         :=  pg_Pricelist_DataLoop.v_Shop_code_new(i);
       v_price_old             :=  pg_Pricelist_DataLoop.v_Price_old(i);
       v_price_new             :=  pg_Pricelist_DataLoop.v_Price_new(i);
       v_srvflag_old           :=  NVL(pg_Pricelist_DataLoop.v_Srvflag_old(i), ' ');
       v_srvflag_new           :=  NVL(pg_Pricelist_DataLoop.v_Srvflag_new(i), ' ');
       v_status_old            :=  NVL(pg_Pricelist_DataLoop.v_Status_old(i), ' ');
       v_status_new            :=  NVL(pg_Pricelist_DataLoop.v_Status_new(i), ' ');
       v_esstockflag_old       :=  NVL(pg_Pricelist_DataLoop.v_Esstockflag_old(i), ' ');
       v_esstockflag_new       :=  NVL(pg_Pricelist_DataLoop.v_Esstockflag_new(i), ' ');
       v_coupon_old            :=  NVL(pg_Pricelist_DataLoop.v_Coupon_old(i),0);
       v_coupon_new            :=  NVL(pg_Pricelist_DataLoop.v_Coupon_new(i),0);
       v_jobtype_old           :=  NVL(pg_Pricelist_DataLoop.v_Jobtype_old(i), '0');
       v_jobtype_new           :=  NVL(pg_Pricelist_DataLoop.v_Jobtype_new(i), '0');
       v_instance_price_old    :=  NVL(pg_Pricelist_DataLoop.v_Instance_price_old(i),0);
       v_instance_price_new    :=  NVL(pg_Pricelist_DataLoop.v_Instance_price_new(i),0);
--0418 v_pjobcode_old          :=  pg_Pricelist_DataLoop.v_Pjobcode_old(i);
--0418 v_pjobcode_new          :=  pg_Pricelist_DataLoop.v_Pjobcode_new(i);
       v_ca_code_old           :=  NVL(pg_Pricelist_DataLoop.v_Ca_code_old(i), ' ');
       v_ca_code_new           :=  NVL(pg_Pricelist_DataLoop.v_Ca_code_new(i), ' ');
       v_goodsnm_old           :=  NVL(pg_Pricelist_DataLoop.v_Goodsnm_old(i), ' ');
       v_goodsnm_new           :=  NVL(pg_Pricelist_DataLoop.v_Goodsnm_new(i), ' ');
       v_url_old               :=  NVL(pg_Pricelist_DataLoop.v_Url_old(i), ' ');
       v_url_new               :=  NVL(pg_Pricelist_DataLoop.v_Url_new(i), ' ');
       v_imgurl_old            :=  NVL(pg_Pricelist_DataLoop.v_Imgurl_old(i), ' ');
       v_imgurl_new            :=  NVL(pg_Pricelist_DataLoop.v_Imgurl_new(i), ' ');
       v_imgurlflag_old        :=  NVL(pg_Pricelist_DataLoop.v_Imgurlflag_old(i),'0');
       v_imgurlflag_new        :=  NVL(pg_Pricelist_DataLoop.v_Imgurlflag_new(i),'0');
       v_u_date_old            :=  pg_Pricelist_DataLoop.v_U_date_old(i);
       v_u_date_new            :=  pg_Pricelist_DataLoop.v_U_date_new(i);
--0418 v_searchflag_old        :=  pg_Pricelist_DataLoop.v_Searchflag_old(i);
--0418 v_searchflag_new        :=  pg_Pricelist_DataLoop.v_Searchflag_new(i);
       v_goodsfactory_old      :=  NVL(pg_Pricelist_DataLoop.v_Goodsfactory_old(i), ' ');
       v_goodsfactory_new      :=  NVL(pg_Pricelist_DataLoop.v_Goodsfactory_new(i), ' ');
--0418 v_goodsbrand_old        :=  NVL(pg_Pricelist_DataLoop.v_Goodsbrand_old(i), ' ');
--0418 v_goodsbrand_new        :=  NVL(pg_Pricelist_DataLoop.v_Goodsbrand_new(i), ' ');

--0418 v_subside_old           :=  NVL(pg_Pricelist_DataLoop.v_Subside_old(i),0);
--0418 v_subside_new           :=  NVL(pg_Pricelist_DataLoop.v_Subside_new(i),0);
       v_popular_new           :=  NVL(pg_Pricelist_DataLoop.v_Popular_new(i),0);
            
       v_option_flag2_old      :=  NVL(pg_Pricelist_DataLoop.v_Option_flag2_old(i),'0');
       v_option_flag2_new      :=  NVL(pg_Pricelist_DataLoop.v_Option_flag2_new(i),'0');
             
       v_price_flag_old        :=  NVL(pg_Pricelist_DataLoop.v_Price_flag_old(i),'0');
       v_price_flag_new        :=  NVL(pg_Pricelist_DataLoop.v_Price_flag_new(i),'0');
             
       v_store_flag_old        :=  NVL(pg_Pricelist_DataLoop.v_Store_flag_old(i),'0');
       v_store_flag_new        :=  NVL(pg_Pricelist_DataLoop.v_Store_flag_new(i),'0');
             
       v_ca_code_dept_old      :=  NVL(pg_Pricelist_DataLoop.v_Ca_code_dept_old(i),'0');
       v_ca_code_dept_new      :=  NVL(pg_Pricelist_DataLoop.v_Ca_code_dept_new(i),'0');
       ...
       IF v_modelno_new > 0 THEN
           ...
           IF  v_modelno_old        <> v_modelno_new        OR v_pl_no_old        <> v_pl_no_new        OR v_shop_code_old  <> v_shop_code_new  OR
               ...
               v_instance_price_old <> v_instance_price_new OR v_store_flag_old   <> v_store_flag_new 
               ----------------------------------------------------------------------------------------------------------------------------------------
               -- OR v_goodsbrand_old <> v_goodsbrand_new  -- 0418 제외
               ----------------------------------------------------------------------------------------------------------------------------------------
               THEN
                   ------------------------------------------------------------------------------------------------------------------------------------
                   --백화점 상품 (모델번호가 변경된 경우)
                   ...
                   IF  v_modelno_new > 0 and v_modelno_old <> v_modelno_new THEN 
                       ...
                       UPDATE /*+ append NOLOGGING */ TBL_SAD_GOODS 
                          SET modelno =  v_modelno_new
                        WHERE pl_no   =  v_pl_no_new 
                          AND modelno <> v_modelno_new;
                       END;
                   ...
                   ---------------------------------------------------------------------------------------
                   -- 0505 이주현
                   -- tbl_goods_middle_list 사용안함 주석처리
                   ---------------------------------------------------------------------------------------                   
--                   v_service_count := 0;
...
--                                   BEGIN
--                                   DELETE tbl_goods_middle_list 
--                                   WHERE  modelno = v_modelno_group_new;
--                                   END;
--
--                               END IF;
--                               
...
--                   END IF;
                   ...
                   -------------------------------------------------------------------------------------------------------------------
                       BEGIN
                           ...
                           SELECT NVL(MIN(p.instance_price), 0)
                                , NVL(COUNT(p.shop_code),0)
                             INTO v_minprice3_new
                                , v_mallcnt3_new
                             FROM TBL_PRICELIST p
                            WHERE p.modelno = v_modelno_new
                              AND p.status  in ('0','8','9')
                       ...
                       BEGIN
                           ...
                           SELECT NVL(AVG(p.price), 0)
                             ...
                             INTO v_avgprice_new
                                ...
                                , v_minprice2_new
                                , v_maxprice_new
                                , v_mallcnt_new -- 16.12.22 lsm v_mallcnt_new 추가
                   ...
                   END IF; 
                       ...
                       IF v_minprice4_new >=0 THEN
                           IF v_apflag_new = '0' THEN
                              BEGIN
                               UPDATE /*+ append NOLOGGING */ TBL_GOODS_SUM   --0418 TBL_GOODS --> TBL_GOODS_SUM
                                  SET  avgprice  = v_avgprice_new
                                   ...
                                   --, rsiflag   = v_rsiflag_tmp  --0418 rsiflag : GOODS_SUM 에 없음
                                     , moddate   = sysdate
                                WHERE modelno    = v_modelno_new;
                               END;
                             ...
                             BEGIN
                               UPDATE /*+ append NOLOGGING */ TBL_GOODS_SUM   --0418 TBL_GOODS --> TBL_GOODS_SUM
                                 ...
                                 WHERE  modelno   =  v_modelno_new
                                  ...
                                  --OR  rsiflag   <> v_rsiflag_tmp   --0418 rsiflag : GOODS_SUM 에 없음
                               ...
                               --------------------------------------------------------------------------------------------
                               -- 0418 한용규 : TBL_GOODS_SUM Update 여부 확인(변경일시만 Update ?) 추가 검토 필요 
                               --               상기 조건과 동일한 기준 적용 여부 확인 ?
                               -- 0505 이주현 : tbl_goods_sum 데이터가 변경되었음을 판단할수 있는 컬럼 추가 필요?
                               --------------------------------------------------------------------------------------------
                               BEGIN
                               UPDATE TBL_GOODS A
                           ...
                           IF v_apflag_new = '0' THEN
                               ...
                               UPDATE /*+ append NOLOGGING */ TBL_GOODS_SUM  --0418 TBL_GOODS --> TBL_GOODS_SUM
                                  SET avgprice  = v_avgprice_new
                                    ...
                                    , mallcnt   = v_mallcnt_new
                                    , mallcnt3  = v_mallcnt3_new
                                  --, rsiflag   = v_rsiflag_tmp  --0418 rsiflag : GOODS, GOODS_SUM 에서 삭제됨 
                                    , moddate   = sysdate
                                WHERE modelno   = v_modelno_new;
                               END;
                               ...
                               UPDATE /*+ append NOLOGGING */ TBL_GOODS_SUM  --0418 TBL_GOODS --> TBL_GOODS_SUM
                                ...
                                WHERE modelno   =  v_modelno_new
                                  AND (minprice <> v_minprice_new 
                                   OR mallcnt   <> v_mallcnt_new 
                                   OR mallcnt3  <> v_mallcnt3_new 
                                 --OR rsiflag   <> v_rsiflag_tmp  --0418 rsiflag : GOODS, GOODS_SUM 에서 삭제됨 
                                   OR minprice = v_price_new 
                                   OR NVL(minprice3,0) <> v_minprice3_new 
                                   OR NVL(minprice4,0) <> v_minprice4_new 
                                   );
                   ...
                   -------------------------------------------------------------------------------------------------------------------
                       ...
                       SELECT NVL(AVG(p.price), 0)
                          ...
                          --, MIN(p.price + DECODE(deliveryinfo, '무료배송', 0, NVL(deliveryinfo2, 2500)))
                            , MIN(CASE WHEN p.deliverytype2=0 THEN (p.price + 2500) 
                                       ELSE (p.price + DECODE(deliveryinfo, '무료배송', 0, NVL(deliveryinfo2, 2500)))
                                       END)
                            , NVL(MAX(p.price), 0)
                            , count(p.shop_code)
                            --------------------------------------------------------------------------------------- 
       ...
       END IF;
           ...
           IF v_modelno_new <= 0 THEN
               ...
               IF (v_status_new='0' OR v_status_new='8' OR v_status_new='9') THEN
                   IF (v_srvflag_new='0' OR v_srvflag_new='L' OR v_srvflag_new='S' OR v_srvflag_new='Z' OR 
                       v_srvflag_new='2' OR v_srvflag_new='3' OR v_srvflag_new='O' OR v_srvflag_new='D' OR v_srvflag_new='9')  THEN
                       IF v_status_old <> v_status_new OR v_srvflag_old <> v_srvflag_new THEN
                          v_status := 'I';
                       END IF;  
                   END IF;
                   
                   IF (v_srvflag_old='0' OR v_srvflag_old='L' OR v_srvflag_old='S' OR v_srvflag_old='Z' OR 
                       v_srvflag_old='2' OR v_srvflag_old='3' OR v_srvflag_old='O' OR v_srvflag_new='D' OR v_srvflag_old='9')  THEN
               ...
               ELSE
                   ...
                   IF (v_status_old='0'  OR v_status_old='8'  OR v_status_old='9') AND 
                      (v_srvflag_old='0' OR v_srvflag_old='L' OR v_srvflag_old='S' OR v_srvflag_old='Z' OR 
                       v_srvflag_old='2' OR v_srvflag_old='3' OR v_srvflag_old='O' OR v_srvflag_new='D' OR v_srvflag_old='9') THEN 
                                              
                       IF (v_status_new <>'0' AND v_status_new<>'8'  AND v_status_new<>'9') OR 
                          (v_srvflag_new<>'0' AND v_srvflag_new<>'L' AND v_srvflag_new<>'S' AND v_srvflag_new<>'Z' AND v_srvflag_new<>'2' AND
                           v_srvflag_new<>'3' AND v_srvflag_new<>'O' AND v_srvflag_new<>'D' ) THEN
                           v_status := 'D';
                       END IF;
   ...
   pg_Pricelist_DataLoop.p_index := 0;
   i := 1;
...
--"DBENURI"."TR_SEN_UPPRICELIST" AFTER UPDATE ON dbenuri.TBL_PRICELIST
--REFERENCING NEW AS NEW OLD AS OLD
--DECLARE
--       v_pl_no_old               NUMBER  := 0;
--       v_pl_no_new               NUMBER  := 0;
...
--                               /* 0502 수정 필요 
--                                  AND EXITS (select 1 
--                                               from TBL_GOODS_SUM B
--                                              where B.modelno   =  B.MODELNO
--                                                AND B.modelno   =  v_modelno_new
--                                                AND (B.minprice <> v_minprice_new 
--                                                 OR B.mallcnt   <> v_mallcnt_new 
--                                                 OR B.mallcnt3  <> v_mallcnt3_new 
--                                               --OR rsiflag   <> v_rsiflag_tmp   --0418 rsiflag : GOODS_SUM 에 없음
--                                                 OR B.minprice=v_price_new 
--                                                 \* 배송비 포함 최저가 nova23 2016.06.21*\
--                                                 OR B.minprice2 <> v_minprice2_new 
--                                                 OR NVL(B.minprice3,0) <> v_minprice3_new 
--                                                 OR NVL(B.minprice4,0) <> v_minprice4_new 
--                                                );*/
--                               --------------------------------------------------------------------------------------------
-- 
--                            END IF;
...
--                       /* 배송비 포함 최저가 nova23 2016.06.21*/
--                       v_store_flag_new,v_instance_price_new,v_minprice2_new,v_ca_code_dept_new,v_minprice4_new,v_brand,
--                       -------------------------------------------------------------------------------------------------------------------------------
--                       -- v_goodsbrand, --0418 제외
--                       -------------------------------------------------------------------------------------------------------------------------------
--                       v_instance_price_new);
--           END IF;
--           -------------------------------------------------------------------------------------------------------------------------------------------
--       i := i + 1;
--     ELSE
...
--        v_instance_price_old    :=  NVL(pg_Pricelist_DataLoop.v_Instance_price_old(i),0);
--      v_instance_price_new    :=  NVL(pg_Pricelist_DataLoop.v_Instance_price_new(i),0);
--        v_pjobcode_old          :=  pg_Pricelist_DataLoop.v_Pjobcode_old(i);
--        v_pjobcode_new          :=  pg_Pricelist_DataLoop.v_Pjobcode_new(i);
--        v_ca_code_old           :=  NVL(pg_Pricelist_DataLoop.v_Ca_code_old(i), ' ');
--        v_ca_code_new           :=  NVL(pg_Pricelist_DataLoop.v_Ca_code_new(i), ' ');
--        v_goodsnm_old           :=  NVL(pg_Pricelist_DataLoop.v_Goodsnm_old(i), ' ');
--        v_goodsnm_new           :=  NVL(pg_Pricelist_DataLoop.v_Goodsnm_new(i), ' ');
...
--        v_u_date_old            :=  pg_Pricelist_DataLoop.v_U_date_old(i);
--        v_u_date_new            :=  pg_Pricelist_DataLoop.v_U_date_new(i);
--        v_searchflag_old        :=  pg_Pricelist_DataLoop.v_Searchflag_old(i);
--        v_searchflag_new        :=  pg_Pricelist_DataLoop.v_Searchflag_new(i);
--        v_goodsfactory_old      :=  NVL(pg_Pricelist_DataLoop.v_Goodsfactory_old(i), ' ');
--        v_goodsfactory_new      :=  NVL(pg_Pricelist_DataLoop.v_Goodsfactory_new(i), ' ');
--        v_goodsbrand_old        :=  NVL(pg_Pricelist_DataLoop.v_Goodsbrand_old(i), ' ');
--        v_goodsbrand_new        :=  NVL(pg_Pricelist_DataLoop.v_Goodsbrand_new(i), ' ');
--             
--        v_subside_old           :=  NVL(pg_Pricelist_DataLoop.v_Subside_old(i),0);
--        v_subside_new           :=  NVL(pg_Pricelist_DataLoop.v_Subside_new(i),0);
--        v_popular_new           :=  NVL(pg_Pricelist_DataLoop.v_Popular_new(i),0);
--             
--             v_option_flag2_old      :=  NVL(pg_Pricelist_DataLoop.v_Option_flag2_old(i),'0');
--        v_option_flag2_new      :=  NVL(pg_Pricelist_DataLoop.v_Option_flag2_new(i),'0');
...
ALTER TRIGGER "DBENURI"."TR_SEN_UPPRICELIST" ENABLE