/* *************************************************************************
 * NAME : dbo.udp_jca_ap_report_bid_sel_daily_sum
 * TYPE : PROCEDURE (SQL_STORED_PROCEDURE)
 * TIME : Create: 2013-08-21 09:58:55.673
 *        Modify: 2018-05-03 17:23:35.923
 *        Backup: 20180521_1739
 ************************************************************************* */

CREATE PROC udp_jca_ap_report_bid_sel_daily_sum
...
AS 
	...
	*/
	...
	select '', data_opt,
		...
		,		[day_sum] = sum(case num when 1 then sales when 2 then round(sales/sale_cnt,0) when 3 then exp_cnt when 4 then hit_cnt when 5 then null when 6 then null when 7 then null end)	
		...
		from #tbl_groupby_day_2 cross join (select 1 num union all select 2 union all select 3 union all select 4 union all select 5 union all select 6 union all select 7) bb
		where sale_date >= @sale_date and sale_date < dateadd(m, 1, @sale_date)
		group by num
		) a 
		order by num
