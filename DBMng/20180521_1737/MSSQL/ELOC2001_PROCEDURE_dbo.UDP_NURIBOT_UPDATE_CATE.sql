/* *************************************************************************
 * NAME : dbo.UDP_NURIBOT_UPDATE_CATE
 * TYPE : PROCEDURE (SQL_STORED_PROCEDURE)
 * TIME : Create: 2013-08-16 16:13:00.24
 *        Modify: 2017-12-20 14:17:38.79
...
CREATE   PROCEDURE [dbo].[UDP_NURIBOT_UPDATE_CATE]
...
BEGIN
	...
	If @strVcode = '55'
		Begin
			select top 1 @ChkNo = no from NURIBOTDB.dbo.TBL_NB_SHOPSTATICS with(readuncommitted) where vcode = 55 and STATUSFLAG = 'U' order by NO DESC
		...
		Begin
			select top 1 @ChkNo = no from NURIBOTDB.dbo.TBL_NB_SHOPSTATICS with(readuncommitted) where vcode = 536 and STATUSFLAG = 'U' order by NO DESC
			...
			WHILE @CateInt <= @CateCnt
				Begin	
					...
					on pl_goodscode = goodscode and pl_goodscode_cs = goodscode_cs
					where pl_vcode = 536 and pl_goodsnm <> '' and goodsnm is not null and pl_modelno > 0 and pl_srvflag <> 'B'
					and (pl_goodsnm <> goodsnm or pl_price <> price or pl_url <> url or isnull(pl_imgurl,'') <> imgurl or pl_account_yn <> account_yn
			...
			WHILE @CateInt <= @CateCnt
				Begin	
					...
					from dbo.pricelist with(readuncommitted) inner join NB01.dbo.TBL_DATA_UPDATE_536 with(readuncommitted)  
					on pl_goodscode = goodscode and pl_goodscode_cs = goodscode_cs
					where pl_vcode = 536 and pl_goodsnm <> '' and goodsnm is not null and pl_modelno < 1 and pl_srvflag <> 'B'
					and (pl_goodsnm <> goodsnm or pl_price <> price or pl_url <> url or isnull(pl_imgurl,'') <> imgurl or pl_account_yn <> account_yn
					or pl_goodsfactory <> factory or isnull(pl_deliveryinfo,'') <> dbo.UDF_RTN_DELIVERYINFO(isnull(deliveryinfo,'')) or pl_freeinterest <> (case when freeinterest <> '' then '1' else '' end) or isnull(pl_price_card,0) <> price_card or isnull(pl_instance_price,0) <> isnull(price_mobile,0)
					or isnull(pl_category,'') = '' or pl_status in ('3','5'))
					and pl_nochange <> '1' and cate = @CATENAME option(maxdop 0)
		...
		Begin
			select top 1 @ChkNo = no from NURIBOTDB.dbo.TBL_NB_SHOPSTATICS with(readuncommitted) where vcode = 4027 and STATUSFLAG = 'U' order by NO DESC
			...
			WHILE @CateInt <= @CateCnt
				Begin
					...
					on pl_goodscode = goodscode and pl_goodscode_cs = goodscode_cs
					where pl_vcode = 4027 and pl_goodsnm <> '' and goodsnm is not null and pl_modelno > 0 and pl_srvflag <> 'B'
					and (pl_goodsnm <> goodsnm or pl_price <> price or pl_url <> url or isnull(pl_imgurl,'') <> imgurl or pl_account_yn <> account_yn
					or pl_goodsfactory <> factory or isnull(pl_deliveryinfo,'') <> dbo.UDF_RTN_DELIVERYINFO(isnull(deliveryinfo,'')) or pl_freeinterest <> (case when freeinterest <> '' then '1' else '' end) or isnull(pl_price_card,0) <> price_card or isnull(pl_instance_price,0) <> isnull(price_mobile,0)
					or isnull(pl_category,'') = '' or pl_status in ('3','5'))
					and pl_nochange <> '1' and cate = @CATENAME option(maxdop 0)
			...
			WHILE @CateInt <= @CateCnt
				Begin
					...
					from dbo.pricelist with(readuncommitted) inner join NB01.dbo.TBL_DATA_UPDATE_4027 with(readuncommitted)
					on pl_goodscode = goodscode and pl_goodscode_cs = goodscode_cs
					where pl_vcode = 4027 and pl_goodsnm <> '' and goodsnm is not null and pl_modelno < 1 and pl_srvflag <> 'B'
					and (pl_goodsnm <> goodsnm or pl_price <> price or pl_url <> url or isnull(pl_imgurl,'') <> imgurl or pl_account_yn <> account_yn
					or pl_goodsfactory <> factory or isnull(pl_deliveryinfo,'') <> dbo.UDF_RTN_DELIVERYINFO(isnull(deliveryinfo,'')) or pl_freeinterest <> (case when freeinterest <> '' then '1' else '' end) or isnull(pl_price_card,0) <> price_card or isnull(pl_instance_price,0) <> isnull(price_mobile,0)
					or isnull(pl_category,'') = '' or pl_status in ('3','5'))
					and pl_nochange <> '1' and cate = @CATENAME option(maxdop 0)
		...
		Begin
			select top 1 @ChkNo = no from NURIBOTDB.dbo.TBL_NB_SHOPSTATICS with(readuncommitted) where vcode = 5910 and STATUSFLAG = 'U' order by NO DESC
			...
			WHILE @CateInt <= @CateCnt
				Begin
					...
					on pl_goodscode = goodscode and pl_goodscode_cs = goodscode_cs
					where pl_vcode = 5910 and pl_goodsnm <> '' and goodsnm is not null and pl_modelno > 0
			...
			WHILE @CateInt <= @CateCnt
				Begin
					...
					from dbo.pricelist with(readuncommitted) inner join NB01.dbo.TBL_DATA_UPDATE_5910 with(readuncommitted)
					on pl_goodscode = goodscode and pl_goodscode_cs = goodscode_cs
					where pl_vcode = 5910 and pl_goodsnm <> '' and goodsnm is not null and pl_modelno < 1
					and (pl_goodsnm <> goodsnm or pl_price <> price or isnull(pl_deliveryinfo,'') <> dbo.UDF_RTN_DELIVERYINFO(isnull(deliveryinfo,'')) or isnull(pl_coupon,0) <> isnull(coupon,0) or isnull(pl_price_card,0) <> price_card or isnull(pl_instance_price,0) <> isnull(price_mobile,0)
					or isnull(pl_category,'') = '' or pl_status in ('3','5')) 
					and pl_nochange <> '1' and cate = @CATENAME option(maxdop 0)
	...
	UPDATE NURIBOTDB.dbo.TBL_NB_SHOPSTATICS_LIST SET STEP_E3_2 = GETDATE() WHERE NO = @strChkNo	
