/* *************************************************************************
 * NAME : dbo.UDP_NURIBOT_TOTAL_STEP1
 * TYPE : PROCEDURE (SQL_STORED_PROCEDURE)
 * TIME : Create: 2011-11-11 14:00:39.993
 *        Modify: 2015-05-15 03:31:33.97
...
CREATE   PROCEDURE [dbo].[UDP_NURIBOT_TOTAL_STEP1]
...
BEGIN
	...
	If @strVcode = '536'
		begin
			insert into @U1(plno, goodsnm, price, url, imgurl, account_yn, deliveryinfo, factory, freeinterest, cate) 
			...
			--and pl_dutycode = @strDutyCode option(maxdop 0)
	...
	--else If (@strVcode = '4027' and DATEPART(hh,getdate()) < 12)
		begin 
			...
			--If @Day = '목요일_임시삭제'
			--	begin
					insert into @U1(plno, goodsnm, price, url, imgurl, account_yn, deliveryinfo, factory, freeinterest, cate, price_card) 
		...
		begin
			...
			--else
			--	begin
					insert into @U1(plno, goodsnm, price, url, imgurl, account_yn, deliveryinfo, factory, freeinterest, cate, price_card, tag_price, call_plan, in_fee_yn, ship_fee_yn, bond_fee, penality_fee) 
	...
	--WHILE @MultiInt < @ModCnt
		BEGIN
			...
			--from dbo.pricelist with(readuncommitted) inner join (select * from @U1 where IDX = @MultiInt) a
