/* *************************************************************************
 * NAME : dbo.UDP_NURIBOT_TOTAL_20
 * TYPE : PROCEDURE (SQL_STORED_PROCEDURE)
 * TIME : Create: 2017-08-04 15:32:24.283
 *        Modify: 2017-08-04 15:32:24.283
...
CREATE   PROCEDURE [dbo].[UDP_NURIBOT_TOTAL_20]
...
BEGIN
	...
	If @strVcode = '5910'
		Begin
			select top 1 @strChkNo = no from NURIBOTDB.dbo.TBL_NB_SHOPSTATICS with(readuncommitted) where vcode = 5910 and STATUSFLAG = 'T' and AllCNT > 0 and MODCNT > -1 and ADDCNT = 0 and DELCNT = 0 order by NO DESC
			...
			If @strDutyCode = 0
				Begin	
					...
					--삭제
					select top 1 @NowCnt = ALLCNT from NURIBOTDB.dbo.TBL_NB_SHOPSTATICS with(readuncommitted) where VCODE = @strVcode and STATUSFLAG = 'T' and MODCNT > 0 and ADDCNT > 0 and DELCNT > 0	order by NO desc
			...
			select top 1 @strChkNo = no from NURIBOTDB.dbo.TBL_NB_SHOPSTATICS with(readuncommitted) where vcode = 5910 and STATUSFLAG = 'T' and AllCNT > 0 and ADDCNT = -1 and DELCNT = -1 order by NO DESC
			WHILE @CateInt <= @CateCnt
				Begin
					set @CATENAME = ''	
					select @CATENAME = cate from @CATE where idx = @CateInt
