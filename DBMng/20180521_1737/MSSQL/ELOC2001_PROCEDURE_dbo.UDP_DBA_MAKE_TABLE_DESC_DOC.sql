/* *************************************************************************
 * NAME : dbo.UDP_DBA_MAKE_TABLE_DESC_DOC
 * TYPE : PROCEDURE (SQL_STORED_PROCEDURE)
 * TIME : Create: 2009-05-08 17:46:05.543
 *        Modify: 2018-05-03 17:23:34.607
 *        Backup: 20180521_1737
 ************************************************************************* */

CREATE PROC UDP_DBA_MAKE_TABLE_DESC_DOC
AS
	-- ----------------------------------------------------------------------------------
	-- 작성자 : wookki25 / 2009.05.08
	-- 설  명 : 테이블 내역서 Html
	-- ----------------------------------------------------------------------------------
	set nocount on
	...
	begin
		--table header
		select @Output =  '<tr><th align="left" style="color:red"><a name="' + Type + ':' + name + '"></a><b>' + Type + ':' + name + '</b></th></tr>',  @description = [description]
				from #Tables where id = @i

		print '<br /><br /><br /><table border="0" cellspacing="0" cellpadding="0" width="750px"><tr><td align="right"><a href="#index">Index</a></td></tr>'
		print @Output
		print '</table><br />'
		print '<table border="0" cellspacing="1" cellpadding="0" width="750px"><caption>Description</caption><tr><td>' + replace(isnull(@description, ''), char(13)+char(10), '<br />') + '</td></tr></table><br />'
		...
		print '<table border="0" cellspacing="1" cellpadding="0" width="750px"><caption>Table Columns</caption><tr><th>Sr.</th><th>Name</th><th>Datatype</th><th>Nullable</th><th>Description</th></tr>'

		While(@j <= @maxj)
		begin
			select @Output = '<tr><td width="20px" align="center">' + Cast((@j) as varchar) + '</td><td width="150px">' + isnull(name,'')  + '</td><td width="150px">' +  upper(isnull(Type,'')) + '</td><td width="50px" align="center">' + isnull(Nullable,'N') + '</td><td>' + replace(isnull([description], ''), char(13)+char(10), '<br />') + '</td></tr>'
	...
	--		FOR 2005
		...
		begin
			...
			print '<table border="0" cellspacing="1" cellpadding="0" width="750px"><caption>Default Constraints</caption><tr><th>Sr.</th><th>Name</th><th>Column</th><th>Value</th></tr>'
		...
		begin
			...
			print '<table border="0" cellspacing="1" cellpadding="0" width="750px"><caption>Indexes</caption><tr><th>Sr.</th><th>Name</th><th>Type</th><th>Columns</th></tr>'
			set @Output = ''
			set @last = ''
			set @current = ''
			While(@j <= @maxj)
