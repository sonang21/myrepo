/* *************************************************************************
 * NAME : dbo.UDP_MP_CATALOG_ATTRIBUTE_BATCH_DEL
 * TYPE : PROCEDURE (SQL_STORED_PROCEDURE)
 * TIME : Create: 2016-04-19 13:06:01.957
 *        Modify: 2018-05-03 17:23:34.897
 *        Backup: 20180521_1737
 ************************************************************************* */


CREATE PROC [DBO].[UDP_MP_CATALOG_ATTRIBUTE_BATCH_DEL]
	@CATEGORY VARCHAR(12),
	@CATEFLAG VARCHAR(1),
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF @DIVIDE_CHAR <> ''
				SET @QRY = @QRY + ' ATTRIBUTE_VALUE = REPLACE(REPLACE(REPLACE(''§'' + REPLACE(LTRIM(RTRIM(''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''' + REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(ATTRIBUTE_VALUE, ''''))), '' ' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''', ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + '''), ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ' '', ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''') + ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''')), LTRIM(RTRIM(''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + REPLACE(@ATTRIBUTE_VALUE, '''', '''''') + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''')), ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''') + ''§'', ''§' + REPLACE(@DIVIDE_CHAR, '''', '''''') + '§'', ''''), ''§' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''', ''''), ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + '§'', '''')'
			ELSE
				SET @QRY = @QRY + ' ATTRIBUTE_VALUE = NULL'
			...
			IF @DIVIDE_CHAR <> ''
				SET @QRY = @QRY + ' AND CHARINDEX(LTRIM(RTRIM(''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + REPLACE(@ATTRIBUTE_VALUE, '''', '''''') + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''')), LTRIM(RTRIM(''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''' + REPLACE(REPLACE(LTRIM(RTRIM(ISNULL(ATTRIBUTE_VALUE, ''''))), '' ' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''', ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + '''), ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ' '', ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + ''') + ''' + REPLACE(@DIVIDE_CHAR, '''', '''''') + '''))) > 0'
			ELSE
				SET @QRY = @QRY + ' AND ATTRIBUTE_VALUE = ''' + REPLACE(@ATTRIBUTE_VALUE, '''', '''''') + ''''
