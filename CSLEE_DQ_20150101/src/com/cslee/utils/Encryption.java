...
{
	//16bytes Key
	final static String DQMS_SKEY = "DQMSSecretKey091";
	
	public static String encrypt(String strClearText) 
	{
		try 
		{
			return encrypt(DQMS_SKEY, strClearText);
		} 
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public static String decrypt(String strEncrptedText) 
	{
		try 
		{
			return decrypt(DQMS_SKEY, strEncrptedText);
		}
		catch (Exception e) 
		{
			e.printStackTrace();
	...
	public static void main(String[] args) {
		if(args.length < 1) 
		{
			System.out.println("usage: java com.cslee.utils.Encryption [text to encrypt]");
			return;
		}
		try {
