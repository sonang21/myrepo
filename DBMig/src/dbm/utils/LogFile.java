...
public class LogFile {
	...
	public LogFile() 
	{
		...
		this.m_sLogFileName = sPath + File.pathSeparator + sFile;
		this.m_logLevel = LOG_LEVEL.valueOf(sLevel);
	...
	public synchronized void logWrite(LOG_LEVEL logLevel, String sMsg) {
		...
		if (fileOpen()) {
			...
			//this.m_oPrintStream.flush();
			System.out.println("===> " + String.valueOf(logLevel.ordinal()) + " : " + String.valueOf(LOG_LEVEL.ERROR.ordinal()));
			if (logLevel.ordinal() <= LOG_LEVEL.ERROR.ordinal()) {
				System.err.println("["+this.m_oDateFormat.format(new Date().getTime()) + "]: " + sMsg);
			}
		}
	...
	public synchronized void logWrite(Exception e) 
	{
		if (this.fileOpen()) {
			...
			e.printStackTrace(this.m_oPrintStream);  // to logfile
	...
	{
		LogFile lf = new LogFile("./log/default.log", "ALL", true);
